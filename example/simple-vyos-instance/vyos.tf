module "vyos_instance" {
    ...
    vyos_configuration_content  = file("vyos.config") # Note: you can use a templatefile function to apply scripting logic to the configuration
                                                      # file and drive some initial parameters via terraform. In this specific example, we do not 
                                                      # do that. Instead, we hardcode the same values used in here (e.g. listen address for web-proxy).

    # One could also use the user-data configuration section runing within cloud-init
    ...
    networks_configuration = {
      ...
      0 = {
        ...
        network_ip          =  local.external_subnet_ip
      ...
      1 = {
        ...
        network_ip          = local.internal_subnet_ip
