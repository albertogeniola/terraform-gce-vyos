...
resource "google_compute_instance" "internal_instance" {
  ...
  echo "export http_proxy=\"http://${local.proxy_ilb_address}:3128/\"">>/etc/profile.d/proxy.sh
  echo "export https_proxy=\"http://${local.proxy_ilb_address}:3128/\"">>/etc/profile.d/proxy.sh
  echo "export ftp_proxy=\"http://${local.proxy_ilb_address}:3128/\"">>/etc/profile.d/proxy.sh
  ...
  curl -x http://${local.proxy_ilb_address}:3128 -o /root/proxy-test.json https://api.ipify.org?format=json
  EOF
